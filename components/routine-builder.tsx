...
import type { WeeklyRoutine, RoutineDay, RoutineExercise, RoutineSection } from "@/lib/types"
...
import { SECTION_ORDER, SECTION_META, normalizeSection, groupBySection } from "@/lib/routine-sections"
...
export function RoutineBuilder({ dataVersion, onUpdate, onStartWorkout, onStartSession, onShowRunning }: RoutineBuilderProps) {
  ...
  function handleAddExercise(dayIndex: number, exerciseId: string) { if (!editingRoutine) return; const day = editingRoutine.days[dayIndex]; const newEx: RoutineExercise = { exerciseId, sets: 3, reps: "8-12", rpe: 7, section: "principal" }; const newDays = [...editingRoutine.days]; newDays[dayIndex] = { ...day, exercises: [...day.exercises, newEx] }; setEditingRoutine({ ...editingRoutine, days: newDays }) }
  ...
  function handleWodChange(dIdx: number, wod: WodBlock | undefined) { if (!editingRoutine) return; const newDays = [...editingRoutine.days]; newDays[dIdx] = { ...newDays[dIdx], wod }; setEditingRoutine({ ...editingRoutine, days: newDays }) }
            ...
            <div className="divide-y divide-border">
              {activeRoutine.days.map((day) => (
                <div key={day.dayNumber} className="p-4">
                  ...
                  <div className="space-y-3 pl-7 border-l-2 border-border ml-2.5">
                    ...
                    {day.wod && (
                      <div className="rounded-lg border border-amber-400/40 bg-amber-500/5 p-2.5 space-y-1">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 flex items-center gap-1">
                          🔥 {formatWodSummary(day.wod)}
                        </p>
                        {day.wod.movements.filter(m => m.name.trim()).map((m) => (
                          <div key={m.id} className="flex justify-between items-center text-xs">
                            <span className="text-foreground/80">{m.name}</span>
                            {m.reps && <span className="text-muted-foreground">{m.reps}</span>}
                          </div>
                        ))}
                        {day.wod.notes && <p className="text-[11px] text-muted-foreground italic pt-0.5">{day.wod.notes}</p>}
                      </div>
                    )}
                    {day.exercises.length === 0 && !day.wod && <span className="text-xs text-muted-foreground italic">Día de descanso</span>}
                  </div>
                </div>
              ))}
            </div>
